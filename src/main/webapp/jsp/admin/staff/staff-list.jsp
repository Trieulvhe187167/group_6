...
<div class="container-fluid">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="get" action="${pageContext.request.contextPath}/admin/staff" class="form-inline" style="width:100%; display:flex; align-items:center; gap:15px; flex-wrap:wrap;">
                ...
                <div class="form-group">
                    ...
                    <select name="role" class="form-control" onchange="this.form.submit()">
                        ...
                        <option value="ROOM_INSPECTOR" ${roleFilter == 'ROOM_INSPECTOR' ? 'selected' : ''}>Room Inspector</option>
                    </select>
                </div>
                ...
                <c:if test="${!isTrashView}">
                    ...
                    <c:if test="${not empty searchKeyword}">
                     ...
                     <a href="${pageContext.request.contextPath}/admin/staff?action=trash" class="btn btn-secondary">
                    <i class="fas fa-trash"></i> View Trash
                </a>
                <a href="${pageContext.request.contextPath}/admin/staff?action=form" class="btn btn-success">
                    <i class="fas fa-plus"></i> Add New Staff
                </a>
                </c:if>
                
            <c:if test="${isTrashView}">
                <a href="${pageContext.request.contextPath}/admin/staff" class="btn btn-primary">
        ...
        <c:choose>
            <c:when test="${not empty staff}">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <small class="text-muted">
                        ...
                        <c:if test="${roleFilter != 'ALL'}"> (${roleFilter} only)</c:if>
                    </small>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <c:forEach var="member" items="${staff}" varStatus="status">
                                <tr>
                                    <td>${(currentPage - 1) * 10 + status.index + 1}</td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar-circle mr-2" style="width: 35px; height: 35px; background: ${member.roleBadgeClass == 'badge-danger' ? '#dc3545' : member.roleBadgeClass == 'badge-primary' ? '#007bff' : member.roleBadgeClass == 'badge-info' ? '#17a2b8' : '#ffc107'}; color: white; display: flex; align-items: center; justify-content: center; border-radius: 50%; font-size: 14px; font-weight: bold;">
                                    ...
                                    <td>
                                        <span class="badge ${member.roleBadgeClass}">
                                    ...
                                    <td>
                                        <c:if test="${!isTrashView}">
                                            <div class="btn-group" role="group">
                                                <a href="${pageContext.request.contextPath}/admin/staff?action=view&id=${member.id}" 
                                                   class="btn btn-sm btn-info" title="View Details">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="${pageContext.request.contextPath}/admin/staff?action=form&id=${member.id}" 
                                                   class="btn btn-sm btn-warning" title="Edit">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <!-- Only show delete button if it's not the current logged-in user -->
                                                <c:if test="${member.id != sessionScope.user.id}">
                                                    <button onclick="confirmDelete(${member.id}, '${member.fullName}')" 
                                                            class="btn btn-sm btn-danger" title="Delete">
                                                        <i class="fas fa-trash"></i>
                                                    </button>
                                                </c:if>
                                                <!-- Show a disabled button with tooltip for current user -->
                                                <c:if test="${member.id == sessionScope.user.id}">
                                        ...
                                        <c:if test="${isTrashView}">
                                            <button onclick="confirmRestore(${member.id}, '${member.fullName}')" 
            ...
            <c:otherwise>
                <div class="text-center py-5">
                    ...
                    <h5 class="text-muted">
                        <c:choose>
                            <c:when test="${isTrashView}">No deleted staff members found</c:when>
                            <c:when test="${not empty searchKeyword}">No staff members found matching "${searchKeyword}"</c:when>
                            <c:when test="${roleFilter != 'ALL'}">No ${roleFilter.toLowerCase()} found</c:when>
                            <c:otherwise>No staff members found</c:otherwise>
                        </c:choose>
                    </h5>
                    <c:if test="${!isTrashView && empty searchKeyword}">
                        <a href="${pageContext.request.contextPath}/admin/staff?action=form" class="btn btn-primary">
...
<div class="modal fade" id="restoreModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to restore staff member "<strong><span id="restoreStaffName"></span></strong>"?</p>
                <div class="alert alert-info">
                    <i class="fas fa-info-circle"></i>
                    This will restore the staff member and make them active again.
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                <a id="restoreConfirmLink" href="#" class="btn btn-success">
                    <i class="fas fa-undo"></i> Restore Staff
                </a>
            </div>
        </div>
    </div>
...
function confirmRestore(staffId, staffName) {
    document.getElementById('restoreStaffName').textContent = staffName;
    document.getElementById('restoreConfirmLink').href = 
        '${pageContext.request.contextPath}/admin/staff?action=restore&id=' + staffId;
    $('#restoreModal').modal('show');
}

function exportStaff(format) {
    // Implementation for export functionality
    const url = '${pageContext.request.contextPath}/admin/staff/export?format=' + format;
